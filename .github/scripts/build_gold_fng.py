...
黄金版 6 因子（滚动 750 日分位 → 加权合成 → 3 日 EMA 平滑）:
...
WEIGHTS = {
    ...
    "athDist": 1.0,     # 距「截至当日」全史高点
    "usd": 0.8,         # 美元强弱（反向，FRED 可用时）
}
PART_ORDER = ["momentum", "accel", "volSpike", "goldSilver", "athDist", "usd"]
...
def build_rows(days, xag_map, usd_by_date):
    ...
    for i in range(WARMUP, n):
        ...
        # ⑤ 距全史高点（≤0，贴近 0 = 贪婪）
        if running_high > 0:
            row["athDist"] = closes[i] / running_high - 1.0
...
def main():
    ...
    rows = build_rows(days, xag_map, usd_by_date)
    if not rows:
