...
黄金版 8 因子（滚动 750 日分位 → 加权合成 → 3 日 EMA 平滑）:
    ① 价格动量  ×2.0  收盘 vs MA50 偏离（业界共识窗长）
    ② 波动率急升 ×1.0  10日/60日 年化波动率之比，反向（急升=恐慌事件）
    ③ 金银比    ×0.9  金银比 60 日变化，反向（比值飙升=只抱黄金=避险恐惧）
    ④ 趋势加速度 ×1.0  20 日动量 − 60 日动量（加速上行=贪婪）
    ⑤ 距历史高点 ×1.0  收盘距「截至当日」全史最高价的回撤，业界线性刻度（非分位）：
                        score = 100·exp(−回撤%/18) —— 贴近高点=贪婪、回撤 10%≈57、
                        回撤 20%≈33。长牛市中分位法会失真（正常回调被读成极端），线性刻度才符合直觉
    ⑥ 趋势质量  ×0.8  20 日内收盘在 MA20 上方的天数占比（单资产版广度）
    ⑦ CFTC 净多头 ×0.5  COMEX 黄金非商业净多头（周频，持仓分位；发布滞后故降权）
...
WEIGHTS = {
    ...
    "volSpike": 1.0,    # 波动率急升（10v60 比，反向）
    "goldSilver": 0.9,  # 金银比 60 日变化（反向）
    "accel": 1.0,       # 趋势加速度（20 日动量 − 60 日动量）
    "athDist": 1.0,     # 距全史高点（业界线性刻度，非分位）
    "breadth": 0.8,     # 趋势质量（20 日内收盘>MA20 占比）
    "cftc": 0.5,        # CFTC 杠杆基金净多头（周频、发布滞后，降权）
...
def main():
    ...
    if len(rows) < 30:
        ...
        sys.exit(1)
    smooth_scores(rows)
    # 注：不做「平滑后重分位」——那会把分布拉伸到全幅、日常波动也标成极端档。
    # 因子集修正后（athDist 线性刻度 + CFTC），合成读数已具备正确的绝对语义。
    ...
    out = {
        ...
        "model": ("黄金版 8 因子加权合成（动量2.0 / 波动率急升1.2 / 金银比1.0 / 趋势加速度1.0 / "
                  "距历史高点0.8·线性刻度 / 趋势质量0.8 / CFTC净多头0.5·周频 / 美元0.8，"
                  "个别数据源不可用时该因子自动缺失降级）；"
                  "均为近 3 年滚动分位（距历史高点除外），输出经 3 日 EMA 平滑；"
